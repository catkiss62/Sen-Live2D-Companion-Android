...
final class SenVtsProfile {
    ...
    boolean hasXiaojingyuExpression() {
        for (String file : activeExpressions) {
            if ("xiaojingyu.exp3.json".equalsIgnoreCase(file)) return true;
        }
        return false;
    }

    String summary() {
        return "VTS参数：" + snapshotName + " · " + parameters.size() + "项"
                + (hasXiaojingyuExpression() ? " · xiaojingyu已开启" : "");
    }
