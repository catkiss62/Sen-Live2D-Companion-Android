...
- 自主待机开启后以两段错开的`wind_sway_soft`原动作连续循环，彻底替换静止站立，不再使用幅度很小的自制叠加；自然眨眼按Sen实机观感从迷梦约0.20秒放缓为0.34秒。常规动作间隔与最近3次防重复参考迷梦，展示动作保持55%明显风摆/31%视频式环绕/14%展示级大摆、首次28～60秒及后续48～100秒。只隐藏已加入待机的动作按钮，其余13条程序动作测试入口恢复。
- 长呆毛强制使用已确认的`高度56 / 宽度83 / 旋转-49 / 左右-16 / 上下+70`，不再点选单一发根。加载时会实际探测三个兔耳输出会驱动的Drawable并全部排除，再从剩余头顶网格选择20个支撑点；跟随头发/头部整体位移、旋转和缩放，仅保留18%呆毛局部柔性。
