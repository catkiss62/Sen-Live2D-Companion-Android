...
- 兔耳“快速摆动两次”继续复用慢眨眼的九轴→原生物理路线：正常物理与隐藏耳朵物理分别计算，只取隐藏物理的`ParamL_angle / ParamR_angle / ParamR_angle2`三项输出。界面可独立调节速度和幅度（50%～250%），当前默认速度150%、幅度100%，供实机确定最终预设。
...
- 预览区默认开启迷梦同路线的极限触屏跟随；摸头范围可在舞台上点选矩形两个对角，界面显示归一化`L/T/R/B`数值供回填，连续滑动触发摸头且10%概率进入大小眼疑惑彩蛋。
- 自主待机开启后始终保留柔风轻晃与约每2.2～5.4秒一次的自然短眨眼；常规动作每5～12秒随机，展示动作按迷梦的低频计时及55%明显风摆/31%视频式环绕/14%展示级大摆随机。近期动作防重复，26个程序动作测试按钮已移除但源码路线和注释保留。
- 长呆毛强制使用已确认的`高度56 / 宽度83 / 旋转-49 / 左右-16 / 上下+70`，不再点选单一发根。运行时自动选择头顶附近20个头发/头部网格支撑点，跟随其整体位移、旋转和缩放，仅保留18%呆毛局部柔性，重点解决大幅点头/摇头时漂浮。
