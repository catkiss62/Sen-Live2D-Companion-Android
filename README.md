...
- 耳鳍角度提供`-50°～+30°`左右联动测试，负数为向外放平；加载时通过已确认有效的耳鳍角度参数自动识别实际变化的Drawable，上下位置不再依赖猜测Part名称。
- 长呆毛通过Cubism Part父子层级取得当前可见子Drawable，提供大小、旋转角度、左右、上下四项精调；所有数值仅在运行时应用。
