...
### v0.3.2-vts-frozen-parity（本轮制作待确认）

- [ ] 同时选择 `Sen.vts-profile.json` 与当前 `.vtube.json` 后，界面显示“VTS冻结参数581项”和ArtMesh颜色匹配数。
- [ ] 冻结模式不启动任何表达式、物理或每帧参数更新，ZIP表情按钮会明确提示本模式停用。
- [ ] `Warning2=-1`等超出模型声明范围的采集值通过Core直写保留，右上白块消失。
- [ ] 对照检查五项：刘海完整、嘴部正常、左右手指完整且无浮空白线、白色丝袜透明度正确、旧版脸周半透明暗红圈不复发。
- [ ] “调整模型”开启后可单指拖动、双指缩放；关闭后锁定，一键可还原。
- [ ] 若上述五项仍失败，记录冻结模式显示的应用/缺失/越界计数；届时才能判定需要Drawable定位或更换底层。
...
1. 构建并实机验证 v0.3.2冻结状态：导入原模型ZIP，再同时导入 `Sen.vts-profile.json` 与当前 `.vtube.json`，只核对五项静态异常。
2. 若冻结状态正确，证明底层可用；随后实现“永久外观层→临时表情层→输入/动作→物理”的正确更新顺序，并从冻结快照逐步移除动态值。
3. 若冻结状态仍错误，记录应用/缺失/越界计数，再启用Drawable定位和Core/Framework兼容检查；不再反复改普通表达式参数。
