...
- Android：`v0.3.4-mask-quality-customize-test`，**本轮制作、待实机确认**。
- Windows：`Sen VTS Parameter Capture v0.1.0`，**核心采集流程已电脑确认**。
- 固定顺序：静态部件缺失/错层已经由C高精度蒙版解决；本轮先提高C蒙版边缘质量，并把耳鳍角度与长呆毛几何做成可撤销的独立试调。实机确认以后，才恢复物理、九轴、表情、动作和AI功能。

## 最近构建记录

### 2026-08-30 · v0.3.4 C蒙版清晰度、耳鳍与呆毛试调（本轮制作、待实机确认）
...
- 构建成功只证明代码与官方Cubism Java接口可编译；1024蒙版边缘、耳鳍角度和两个呆毛网格变换仍为“待实机确认”。
