...
### v0.3.1-vts-layered-appearance（本轮制作待确认）

- [ ] APK覆盖安装后不再读取旧 `Sen.vts-profile.json`；旧参数包即使仍在App私有目录也不会影响模型。
- [ ] “导入外观”可保存并安装 `xiaojingyu.exp3.json`，55项修改作为原模型默认值上的表达式增量运行；未列出的默认部件保持原状。
- [ ] 同一入口可保存当前 `.vtube.json`，解析并按Drawable ID应用79项乘算/屏幕色；界面显示成功匹配数和缺失数。
- [ ] `VTS_Add` 转换为标准 `Add`，水印参数与其他增量正常生效。
- [ ] “应用小鲸鱼完整外观 / 查看原始状态”可以重新加载对照，不再提供误导性的旧快照模式。
...
- [x] 禁止后续AI重新尝试“放开203项过滤”或“每帧写入全部581项”；这会继续破坏底值，并锁死物理、跟随和动作。
...
- 正确静态复现顺序固定为：原模型默认值 → 实际exp3增量 → VTS ArtMesh乘算/屏幕色。未出现在exp3中的参数和部件必须保留原模型默认状态。
- 官方 Cubism Java 能按Drawable ID设置运行时乘算色和屏幕色；不要为了复现VTS颜色修改购买模型PNG，也不要再把581项最终快照每帧写回。
...
1. 构建并实机验证 v0.3.1：先导入原模型ZIP，再导入 `xiaojingyu.exp3.json` 与当前 `.vtube.json`，对照VTS截图核对完整静态外观。
2. 若颜色正确但仍有头顶白块/眼部叠层，记录v0.3.1显示的ArtMesh匹配/缺失数，再单独诊断部件选择或原生Drawable/Mask兼容，不回退旧参数快照路线。
3. 静态画面一致后，处理兔耳角度、鞋子绑腿和蝴蝶结位置；这些属于用户定制任务，不与静态还原混修。
4. 电脑采集器保留用于九轴、输入映射和动态状态诊断，不再负责静态外观底值。
