...
- Android：`v0.3.3-mask-ab-test`，**本轮制作、待实机确认**。
...
- [ ] A“默认单缓冲”能复现v0.3.2的长期异常，作为同一模型、同一参数、同一贴图的基线。
- [ ] B“动态多缓冲”显示实际唯一蒙版组数和自动计算的缓冲数；检查白块、刘海、嘴、手指/指甲、丝袜、耳朵厚度和耳下紫块是否变化。
- [ ] C“高精度蒙版”明确显示高精度已开启；与B分别截图，不能只根据人物远景判断。
- [ ] 切换A/B/C只改变官方Cubism蒙版策略，不改变VTS冻结参数、颜色和模型文件。
